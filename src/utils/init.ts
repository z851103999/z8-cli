...
import ora from 'ora'
import sortDependencies, { currentPackageJson, deepMerge, cliJson, readJsonFile, removeDir } from './common'

export function checkName(name: string) {
  const inCurrent = name === '.'
  const projectName = inCurrent ? path.relative('../', process.cwd()) : name
  const projectPath = inCurrent ? path.join(process.cwd(), name) : path.join(process.cwd(), projectName)
...
/**
  用于下载用户自定义的模板
 */
export function checkTemplate(projectPath: string, isCwd: boolean, git: string[]) {
  if (isCwd) {
  ...
  exec(`git clone -b ${git[1] ?? 'master'} ${git[0]} ${path.basename(projectPath)}`, (err, stdout, stderr) => {
    if (err) {
...
export async function render(projectPath: string, feature: string[]) {
  ...
  let succendMsg = green('初始化完成') + ' 如需使用create命令查看README.md '
